...
// Elementos do DOM (Listas e Botões de Ação)
...
// Elementos do DOM (Formulário de Busca de Filme)
...
// --- 2. FUNÇÕES GERAIS DE UI E AUXILIARES ---
...
// --- 3. ROTINAS DE FILMES (SOMENTE BUSCA EXTERNA) ---
...
    }
    
    console.log(`[Busca Externa Iniciada] Título: ${tituloBusca}`);
    
    if (filmesLista) filmesLista.innerHTML = '<p class="info-msg">Buscando filme em catálogo externo...</p>';

    try {
        const urlBusca = `${FILMES_BUSCA_EXTERNA_ENDPOINT}?titulo=${encodeURIComponent(tituloBusca)}`;
...
    
    try {
        const urlCadastro = CLIENTES_ENDPOINT.endsWith('/') ? CLIENTES_ENDPOINT : `${CLIENTES_ENDPOINT}/`;
...
    mostrarMensagemTopo(`Preparando edição do cliente ID #${clienteId}.`, false);
    
...
                `ID: ${c.id}`,
                `CPF: ${c.cpf}`, 
                `Nascimento: ${dataNascimentoFormatada}`, // Exibe no formato dd/mm/aaaa
                `Email: ${c.email}`,
                `Telefone: ${c.telefone}`
            ];
            
...
// --- 5. INICIALIZAÇÃO DA APLICAÇÃO E BINDINGS ---
